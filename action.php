  ...
  if (mysqli_num_rows($run)==0) {
    ...
    <div class='card-footer bg-info'>
     <a href='profile.php' class='btn btn-outline-danger'>Home</a>
      <a href='#' class='btn btn-outline-danger cancel-ticket' data-ticket-id='{$row['tid']}'>Cancel Ticket</a>
    </div>
  </div>";     
...
if (isset($_POST['cancel_ticket'])) {
  ...
  $uid = $_SESSION['uid']; // Get the logged-in user ID
  
  // Check if the ticket belongs to the user
  $sql = "SELECT * FROM ticket WHERE tid='$ticket_id' AND uid='$uid' AND ticket_status='Confirm'";
  $run = mysqli_query($con, $sql);

  if (mysqli_num_rows($run) > 0) {
      // Update ticket status to 'Cancelled'
      $update_ticket = "UPDATE ticket SET ticket_status='Cancelled' WHERE tid='$ticket_id'";
      $update_run = mysqli_query($con, $update_ticket);

      // Fetch booking details to update vacant seats
      $ticket_data = mysqli_fetch_array($run);
      $bus_id = $ticket_data['bus_id'];
      $no_of_seats = $ticket_data['no_seat'];
      $jdate = $ticket_data['jdate'];

      // Update booking details to increase vacant seats
      $update_booking = "UPDATE booking_det SET vacant = vacant + $no_of_seats 
                         WHERE bus_id='$bus_id' AND jdate='$jdate'";
      $update_booking_run = mysqli_query($con, $update_booking);

      if ($update_run && $update_booking_run) {
          echo "Ticket canceled successfully.";
      } else {
          echo "Error: Unable to cancel the ticket.";
      }
  } else {
      echo "Invalid ticket or already canceled.";
  }
}
